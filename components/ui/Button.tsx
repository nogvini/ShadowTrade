...
const buttonVariants = cva(
  'font-press-start inline-flex items-center justify-center whitespace-nowrap rounded-md ring-offset-background transition-all duration-300 ease-smooth focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 uppercase tracking-wider shadow-md hover:shadow-lg active:shadow-sm',
  {
    variants: {
      variant: {
        default:
          'bg-text-primary text-bg-primary hover:bg-text-primary/90 shadow-text-sm',
        destructive:
          'bg-error text-text-primary hover:bg-error/90 shadow-text-sm',
        outline:
          'border border-text-primary bg-transparent text-text-primary hover:bg-text-primary hover:text-bg-primary shadow-text-sm',
        secondary:
          'bg-bg-secondary text-text-primary hover:bg-bg-secondary/80 shadow-text-sm',
        ghost:
        ...
        success:
          'bg-success text-bg-primary hover:bg-success/90 shadow-text-sm',
      },
      size: {
    ...
    return (
      ...
      >
        {Icon && iconPosition === 'left' && (
          <Icon className={cn(getIconSize(), 'mr-1 sm:mr-2')} />
        )}
        {shadowText ? (
        ...
        {Icon && iconPosition === 'right' && (
          <Icon className={cn(getIconSize(), 'ml-1 sm:ml-2')} />
        )}
      </Comp>
    );
  }
